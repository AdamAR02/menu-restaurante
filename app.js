  ...
  db.ref("platillos")
    ...
    .then((snap) => {
      ...
      db.ref()
        ...
        .then(() =>
          showToast(
            "¡" + PLATILLOS_EJEMPLO.length + " platillos de ejemplo cargados!",
            "success",
          ),
        )
        .catch((e) =>
          showToast("Error al cargar ejemplos: " + e.message, "error"),
        );
    });
}
...
function saveDish() {
  ...
  if (!name) {
    showToast("El nombre del platillo es obligatorio.", "error");
    return;
  }
  if (!price || price <= 0) {
  ...
  if (!db) {
    showToast("Sin conexión con Firebase. Recarga la página.", "error");
    return;
  }
...
function renderMenu(dishes) {
  ...
  const filtered =
    filterCat === "all" ? dishes : dishes.filter((d) => d.cat === filterCat);

  const el = document.getElementById("menu-content");
  if (!filtered.length) {
    el.innerHTML =
      '<div class="empty-state"><span>🍽️</span>No hay platillos en esta categoría todavía.</div>';
    return;
  }
...
function buildFilterBar(dishes) {
  ...
  const catBtns = CATS.filter((c) => activeCats.includes(c.id))
    .map((c) => {
...
function renderAdmin(dishes) {
  ...
  if (!dishes.length) {
    el.innerHTML =
      '<div class="empty-state"><span>✏️</span>Agrega el primer platillo usando el formulario de arriba.</div>';
    return;
  }
