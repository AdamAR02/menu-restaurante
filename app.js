...
function saveDish() {
    ...
    if(!name) { showToast('El nombre del platillos es obligatorio.', 'error'); return;}
    if(!price || price <= 0) { showToast('Ingresa un precio valido mayor a 0.', 'error'); return;}
    if(!db) { showToast('Sin conexion con Firebase. recarga la pagina.', 'error'); return;}
...
.then(() => {showToast('platillo agregado al menu.', 'succes'); clearForm();})
.catch(e => showToast('Error: ' + e.message, 'error'));
    }
}
...
function deleteDish(id, name) {
    if (!confirm('¿Eliminiar "' + name + '"?')) return;
    if (!db) return;
...
if (!filtered.length) {
    el.innerHTML = '<div class="empty-state"><span>🍽️</span>No hay platillos en esta categoria todavia.</div>';
    return;
}
...
el.innerHTML = Object.entries(byCat).map(([catId, items]) => {
    ...
    const cards = items.map(d => `
      ...
      ${d.desc ? `<div class="dish-desc">${esc(d.esc)}</div>` : ''}
